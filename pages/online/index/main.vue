    ...
    <div class="container">
      ...
      <div class="page_container">
        ...
        <view class="list" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend">
          ...
          <sib-list ref="sibList" @isRefresh='isRefresh' @scrolltolowerFn="scrolltolowerFn" :isNotMore="finished" :floterText="notMoreText" :scrollHeight="height">
            <!-- 内部block可自定义 -->
            <block class="" slot="sibScrollList">
              <view class="table">
                ...
                <view class="tr" v-for="(item,index) in list" :key="item.id" :class="{'tr1': index == 0}">
                  <view class="td">{{ item.manager }}</view>
                  <view class="td">{{ item.name }}</view>
                  <view class="td">{{ Math.floor(item.ytd * 100) / 100 + '%' }}</view>
                  <view class="td">{{ Math.floor(item.three_year_profit * 100) / 100 + '%' }}</view>
                  <view class="td">{{ Math.floor(item.roll_year_win * 100) / 100 + '%' }}</view>
                  <view class="td">{{ Math.floor(item.sharpe_ratio * 100) / 100 + '%' }}</view>
        ...
        <view>
          <view class="table">
            <view class="tr" v-for="(item,index) in list4" :key="item.id">
              <view class="td">{{ item.manager }}</view>
              <view class="td">{{ item.name }}</view>
              <view class="td">{{ Math.floor(item.ytd * 100) / 100 + '%' }}</view>
              <view class="td">{{ Math.floor(item.three_year_profit * 100) / 100 + '%' }}</view>
        ...
        <view style="position: fixed;bottom: 0;display: flex" class="bottom">
          <view class="buttons" @click="nav('/pages/analysis/simulate')">构建模拟组合</view>
          <view class="buttons" >指标横向比较</view>
<!--          <view class="buttons" @click="nav('/pages/analysis/across')">指标横向比较</view>-->
        </view>
    ...
    export default {
        ...
        data() {
            return {
                ...
                list2: ["从高到低", "从低到高"],
                type3: "全部策略",
                list3: ["全部策略", "量化", "固收","宏观对冲","纯多头","多空","复合","其他"],
                strategys: {
                    "全部策略": "",
                    "量化": "quantification",
                    "固收": "fixed__income",
