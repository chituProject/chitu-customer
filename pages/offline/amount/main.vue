  ...
  <div class="container page-padding">
    ...
    <div class="box card" style="z-index: 1;" v-for="(item, index) in list" :key="index">
      ...
      <div>
        ...
        <span class="fs-16 right color-main">¥<span class="fs-19">{{ item.offline_order_amount | formatPrice }}</span></span>
      </div>
    </div>
  </div>
</template>
        ...
        success: (res) => {
          if (res.statusCode > 199 && res.statusCode < 300) {
            ...
            for (let i = 0; i < this.list.length; i += 1) {
              ...
              sum += this.list[i].offline_order_amount;
            }
