    ...
    <view class="table_box_big" :style="{ height: tableHeight }">
      <view class="table_box">
        ...
        <template v-if="list.length">
          ...
          <!-- 固定左边一列 】-->
          <!-- 固定右边一列 【-->
          <view v-if="columnsFixedRight[0]" class="fixed-right">
            <view class="tr fixed-thead-tr">
              <view
                class="td"
                :style="{
...
export default {
  ...
  computed: {
    columnsFixedRight() {
      const t = this.columns.filter(item => item.$fixed == "right");
      return t.length ? [t[0]] : [];
    },
    columnsFixedLeft() {
      const t = this.columns.filter(item => item.$fixed == "left");
      return t.length ? [t[0]] : [];
    },
    // 表格高度
    tableHeight() {
    ...
    emptyColHeight() {
      const t = this.height
        ? `${this.height - this.thTdHeight - 30}px`
        : "100px";
  ...
  methods: {
    ...
    countColspanWidth(item, tdItem, index, tdItemIndex, iswrap = false) {
      const borderLeft = iswrap && tdItemIndex > 0 ? 1 : 0;
      // 是否跨列,返回跨列个数，1为不跨列
      const moreThanOne =
    ...
    countHeadColspanWidth(item, index, iswrap = false) {
      const borderLeft = iswrap && index > 0 ? 1 : 0;
      const tmp = item.$width ? parseInt(item.$width) : this.tdWidth;
      return `${tmp}px`;
    },
    /**
     * 计算单列高
